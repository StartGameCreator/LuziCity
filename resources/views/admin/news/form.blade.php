    ...
    <section class="settings-panel" aria-label="Editor de noticia">
        <form method="post" action="{{ $article->exists ? route('admin.news.update', $article) : route('admin.news.store') }}" enctype="multipart/form-data" class="admin-form social-settings-form">
            ...
            <div class="social-settings-grid">
                <label>
                    Titulo
                    <input id="news_title" name="title" value="{{ old('title', $article->title) }}" required>
                </label>
                <label>
                    Slug
                    <input name="slug" value="{{ old('slug', $article->slug) }}" placeholder="gerado automaticamente">
                </label>
                <label>
                    Editoria
            ...
            <label>
                Resumo / chamada
                <textarea id="news_excerpt" name="excerpt" rows="3">{{ old('excerpt', $article->excerpt) }}</textarea>
            </label>
            ...
            <label>
                Texto da noticia
                <textarea id="news_body" name="body" rows="16" required>{{ old('body', $article->body) }}</textarea>
            </label>
