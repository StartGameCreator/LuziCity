...
class AdminNewsController extends Controller
{
    ...
    public function store(Request $request): RedirectResponse
    {
        ...
        $article = NewsArticle::query()->create($this->validatedData($request) + [
            ...
            'published_by' => $request->input('status') === 'published' ? $request->user()->id : null,
        ]);

        return redirect()->route('admin.news.edit', $article)->with('status', 'Notícia salva.');
    }
    ...
    public function update(Request $request, NewsArticle $news): RedirectResponse
    {
        ...
        $data['published_by'] = $data['status'] === 'published' ? $request->user()->id : $news->published_by;

        $news->update($data);

        return back()->with('status', 'Notícia atualizada.');
    }
