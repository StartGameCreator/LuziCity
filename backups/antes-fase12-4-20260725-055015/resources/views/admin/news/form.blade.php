...
@section('content')
    ...
    <section class="content-band">
        ...
        @if($article->exists)<a class="primary-action" href="{{ route('admin.news.workflow.show',$article) }}">Revisão, aprovação e publicação</a>@endif
    </section>
    ...
    <section class="settings-panel" aria-label="Editor de noticia">
        <form method="post" action="{{ $article->exists ? route('admin.news.update', $article) : route('admin.news.store') }}" enctype="multipart/form-data" class="admin-form social-settings-form">
            ...
            <div class="social-settings-grid">
                ...
                <label>
                    Status
                    <input type="hidden" name="status" value="draft"><strong>Rascunho — use o fluxo editorial para aprovar e publicar.</strong>
                </label>
                <label>
                    Data de publicacao
