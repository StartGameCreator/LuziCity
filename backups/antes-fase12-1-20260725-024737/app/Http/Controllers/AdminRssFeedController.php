...
class AdminRssFeedController extends Controller
{
    ...
    private function validatedData(Request $request): array
    {
        $data = $request->validate([
            ...
            'deduplication_strategy' => ['required', 'in:url,guid,url_guid'],
        ]);

        $data['sort_order'] = $data['sort_order'] ?? 0;
        $data['frequency_minutes'] = (int) ($data['frequency_minutes'] ?? 60);
        $data['deduplication_strategy'] = $data['deduplication_strategy'] ?? 'url';
