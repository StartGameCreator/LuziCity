...
async function configurePush() {
    ...
    let config;
    try { config = JSON.parse(configMeta.content); } catch { return; }
    ...
    button.addEventListener('click', async () => {
        ...
        try {
            ...
            await fetch('/push/subscriptions', {
                ...
                body: JSON.stringify({ token, device_name: navigator.userAgent, platform: navigator.platform || 'web' }),
            }).then((response) => { if (!response.ok) throw new Error('Falha ao salvar inscrição.'); });
...
window.addEventListener('load', () => {
    ...
    configurePush().catch(console.error);
});
