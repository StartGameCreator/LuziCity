...
class AiNewsGenerator
{
    public function __construct(
        ...
        private readonly AiEditorialMemoryService $memory
    ) {
    }

    public function generate(array $data): array
    {
        $profile = $this->memory->profile($data['category_id'] ?? null);
        ...
        $variables = [
            ...
            'tone' => $profile->tone,
            'rules' => $this->memory->compile($profile),
        ...
        $memoryNotes = $this->memory->review(implode("\n", [$result['title'] ?? '', $result['subtitle'] ?? '', $result['excerpt'] ?? '', $result['body'] ?? '']), $profile);
        $result['review_notes'] = array_values(array_unique(array_merge((array) ($result['review_notes'] ?? []), $memoryNotes)));
        $result['editorial_memory_profile'] = $profile->name;
