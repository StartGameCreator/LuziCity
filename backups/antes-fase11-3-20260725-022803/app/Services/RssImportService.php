...
class RssImportService
{
    ...
    public function importFeed(RssFeed $feed, int $limit = 12): array
    {
        try {
            ...
            $created = 0;
            $duplicates = 0;
            foreach (collect($this->rssXmlItems($xml))->take(max(1, min(30, $limit, $feed->max_items_per_run ?: 12))) as $item) {
    ...
    private function normalizeItem(\SimpleXMLElement $item, RssFeed $feed): array
    {
        ...
        return [
            ...
            'published_at' => $publishedAt, 'imported_at' => now(), 'is_visible' => ! $feed->require_human_review,
