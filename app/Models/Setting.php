...
class Setting extends Model
{
    ...
    public static function socialLinks(): array
    {
        ...
        foreach ($links as $key => $social) {
            if (! is_array($social)) {
                $links[$key] = [
                    'label' => is_string($social) ? $social : (string) $key,
                    'url' => '#',
                ];
            }
    ...
    public static function enabledSocialLoginProviders(): array
    {
        return collect(self::socialLoginProviders())
            ->filter(fn ($provider) => is_array($provider) && ! empty($provider['enabled']))
            ->all();
    }
    ...
    public static function googleAds(): array
    {
        ...
        $settings = is_array($settings) ? $settings : ['client' => '', 'slots' => []];
        $settings['slots'] = is_array($settings['slots'] ?? null) ? $settings['slots'] : [];
    ...
    public static function radioSettings(): array
    {
        $settings = [
            ...
            'audio_stream_url' => '',
            'schedule_text' => 'Ao vivo com música, notícias, entrevistas e participação dos ouvintes.',
            'field_live_enabled' => false,
            'field_live_title' => 'Cobertura externa ao vivo',
            'field_live_description' => 'Transmissão direto da rua, eventos, campanhas e ações especiais da Luzicity.',
    ...
    public static function trackingPixels(): array
    {
        ...
        $settings = is_array($settings) ? $settings : ['meta_pixel_id' => '', 'tiktok_pixel_id' => ''];
    ...
    {
        $settings = [
            'copyright' => 'Copyright © '.date('Y').' Luzicity. Todos os direitos reservados.',
            'site_name' => 'Luzicity',
            'site_logo' => '',
            'site_favicon' => '',
            'default_share_image' => '',
    ...
    {
        $fallback = 'A Luzicity é uma plataforma de notícias local, criada para aproximar leitores, cidades, jornalistas, colunistas e anunciantes em uma experiência leve, acessível e preparada para crescer.';
    ...
    public static function vehicleTypeOptions(): array
    {
        return [
            'car' => 'Carros',
            'motorcycle' => 'Motos',
            'nautical' => 'Embarcações NÃ¡uticas',
        ];
    }
    ...
    private static function defaultVehicleBrandLogos(string $vehicleType = 'car'): array
    {
        $defaults = [
            'car' => [
                ...
                ['name' => 'Citroën', 'logo_url' => 'https://cdn.simpleicons.org/citroen'],
            ],
            'motorcycle' => [
                ['name' => 'Honda', 'logo_url' => 'https://cdn.simpleicons.org/honda'],
